...
interface VehicleLoanTableProps {
...
export function VehicleLoanTable({
  applications,
  ...
  refreshApplicants,
}: VehicleLoanTableProps) {
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 5

  // Filter applications for vehicle loans only (Auto Loan)
  const vehicleLoanApplications = Object.entries(applications)
    .filter(
      ([_, data]) => data.loanType?.toLowerCase() === "auto loan" || data.loan_type?.toLowerCase() === "auto loan",
    )
    .sort(([, a], [, b]) => {
  ...
  const totalPages = Math.ceil(vehicleLoanApplications.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const paginatedApplications = vehicleLoanApplications.slice(startIndex, startIndex + itemsPerPage)
  ...
  return (
    <Card className="h-full">
      <div className="flex items-center justify-between px-6 pt-6">
        <h2 className="text-xl font-semibold">Vehicle Loans</h2>
        {refreshApplicants && (
      ...
      <CardContent className="pt-4">
        ...
        ) : vehicleLoanApplications.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">No vehicle loan applications found</div>
        ) : (
          <>
                  ...
                  {paginatedApplications.map(([key, data], index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{data.loanType || data.loan_type || "Auto Loan"}</TableCell>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-sm text-muted-foreground">
                  Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, vehicleLoanApplications.length)} of{" "}
                  {vehicleLoanApplications.length} entries
                </p>
