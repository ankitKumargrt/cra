...
interface PersonalLoanTableProps {
...
export function PersonalLoanTable({
  applications,
  ...
  refreshApplicants,
}: PersonalLoanTableProps) {
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 5

  // Filter applications for personal loans only
  const personalLoanApplications = Object.entries(applications)
    .filter(
      ([_, data]) =>
        data.loanType?.toLowerCase() === "personal loan" || data.loan_type?.toLowerCase() === "personal loan",
    )
    .sort(([, a], [, b]) => {
  ...
  const totalPages = Math.ceil(personalLoanApplications.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const paginatedApplications = personalLoanApplications.slice(startIndex, startIndex + itemsPerPage)
  ...
  // Function to display the DTI value (only for Personal Loans)
  const getDtiDisplay = (dti: number | null | undefined, kycStatus: boolean | undefined) => {
    ...
    // Simply display the DTI value
    if (dti !== undefined && dti !== null) {
      // Display the DTI value with a percentage sign
      return <div>{dti}%</div>
    }
  ...
  return (
    <Card className="h-full">
      <div className="flex items-center justify-between px-6 pt-6">
        <h2 className="text-xl font-semibold">Personal Loans</h2>
        {refreshApplicants && (
      ...
      <CardContent className="pt-4">
        ...
        ) : personalLoanApplications.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">No personal loan applications found</div>
        ) : (
          <>
                  ...
                  {paginatedApplications.map(([key, data], index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{data.loanType || data.loan_type || "Personal Loan"}</TableCell>
                        ...
                        <TableCell>{getDtiDisplay(data.dti, data.kyc_status)}</TableCell>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-sm text-muted-foreground">
                  Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, personalLoanApplications.length)} of{" "}
                  {personalLoanApplications.length} entries
                </p>
