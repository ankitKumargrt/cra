...
export function ExtractedDataView({
  ...
  // Combine KYC and ITR content into a single formatted document
  const combinedContent = formatCombinedContent(kycContent, itrContent)

  // Show loading state if loading
  if (isLoading) {
    return <div className="p-4 text-center">Loading extracted data...</div>
  }
...
function formatCombinedContent(kycContent?: string, itrContent?: string): string {
  ...
  try {
    ...
    if (!itrPanCardTable && itrContent) {
      ...
      const tableMatch = itrContent.match(
        /\| Field \| Value \|[\s\S]*?PAN Number[\s\S]*?Name[\s\S]*?(?=\n\n|\n\*\*|$)/i,
      )
      if (tableMatch) {
...
`
    ...
    return "Error occurred while formatting the content."
  }
}
